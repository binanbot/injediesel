...
export function CeoTopProducts({ data, isLoading, title = "Top 10 Produtos por Faturamento" }: Props) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        {isLoading ? <Skeleton className="h-[300px] w-full" /> : data.length === 0 ? (
          <p className="text-center text-muted-foreground py-8">Sem dados de produtos no período</p>
        ) : (
              ...
              {data.map((item, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-border/50">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-xs text-muted-foreground ml-2">({item.count} un.)</span>
                  </div>
                </div>
              ))}
            </div>
